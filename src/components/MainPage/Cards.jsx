...
const cardData = [
  {
    icon: <TbFileSearch className="w-10 plus-jakarta-sans h-10 text-[#0F172A] bg-white rounded-full shadow" />,
    title: "Create a Gig",
    desc: "Got a skill to share or a task to outsource? Drop it here and let the right people find you."
  },
  {
    icon: <CiDeliveryTruck className="w-10 h-10 text-[#0F172A] bg-white rounded-full shadow" />,
    title: "Deliver  Value",
    desc: `Collaborate with real clients, deliver 
     solutions that matter, and grow your credibility through hands-on experience.`
  },
  {
    icon: <GiTakeMyMoney className="w-10 h-10 text-[#0F172A] bg-white rounded-full " />,
    title: "Earn and Grow",
  ...
  return (
    <div className="space-y-6">
      {cardData.map((card, idx) => (
        <div key={idx} className="flex items-center  text-[#0F172A] rounded-xl p-4   ">
          <div>{card.icon}</div>
          <div className="ml-5">
